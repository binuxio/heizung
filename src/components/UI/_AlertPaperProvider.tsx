...
const initialConf: AlertConfig = {
    ...
    message: null,
    messageStyle: {},
    dialogStyle: { borderRadius: 0, backgroundColor: "white" },
    dismissable: true
}
const _AlertPaperProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
        ...
        return (
            <Portal>
                <Dialog dismissable={alertConfig.dismissable || initialConf.dismissable}
                    ...
                    visible={isVisible}>
                    <Dialog.Title style={[{ fontSize: 20 }, alertConfig?.titleStyle]}>{alertConfig?.title}</Dialog.Title>
                    {alertConfig.message && (
                        <Dialog.Content>
...
const useAlert = ({ title, titleStyle, actions, buttonsStyle, message, messageStyle, dismissable, dialogStyle }: AlertConfig = {}) => {
...
export { _AlertPaperProvider, useAlert };
